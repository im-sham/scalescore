...
- `POST /api/v1/assessments/mila/workflow` remains the one-release
  legacy/full-detail Workflow Context compatibility endpoint. It accepts direct
  workflow metadata without requiring dataset CSVs and returns the complete
  `ScaleScoreReport`.
  Optional `workflow_ref` and `control_refs` can be supplied as Proofhouse V0.1
  envelopes from Workflow Context. Canonical `ControlRef` uses the pinned
  generated Contracts binding. Readiness preserves canonical values and omitted
  alternate pins through validation, report serialization, persistence,
  readback, and response; explicit null pins are rejected.
  Before scoring or persistence, `org_id`, the authenticated tenant,
  `workflow_context.workflow_id`, the top-level `WorkflowRef`, every
  `ControlRef`, and every embedded owning `WorkflowRef` must align across
  tenant, environment, and workflow.
  The exact former repository-local `ControlRef` model remains accepted
  inbound and on stored readback for one release. Its objects remain observably
  legacy and are never repaired or converted into canonical `ControlRef` truth;
  malformed canonical data cannot fall back to this branch. Historical
  Pydantic behavior is retained: omitted legacy fields with defaults are
  accepted and those defaults materialize during serialization and
  persistence/readback.
- `POST /api/v1/assessments/mila/workflow/assessment-ref` is the compact
  Workflow-facing transport endpoint. It requires a canonical `workflow_ref`,
  runs and stores the same tenant-scoped full Readiness report, and returns only
...
- Both direct workflow endpoints accept optional `workflow_evidence` to deepen
  ...
  mint owner confirmation, Governance approval, policy/use/release/external or
  gate authority, audit completeness, approval evidence, decision logs, or
  completion authority. Legacy authority-like fields are carried for
  compatibility but never scored.
  Optional `operational_learning_inputs` can be supplied to add a separate
  operational-learning suitability block. Supported v1 inputs are
