...
- `POST /api/v1/assessments/mila/workflow` remains the one-release legacy/full-detail Workflow Context compatibility endpoint. It accepts direct workflow metadata without requiring dataset CSVs and returns the complete `ScaleScoreReport`.
  Optional `workflow_ref` can be supplied as a Proofhouse V0.1 `WorkflowRef` envelope from Workflow Context. Readiness preserves it on `ScaleScoreReport.workflow_ref` and emits `ScaleScoreReport.assessment_ref` inside the full report.
- `POST /api/v1/assessments/mila/workflow/assessment-ref` is the compact Workflow-facing transport endpoint. It requires a canonical `workflow_ref`, runs and stores the same tenant-scoped full Readiness report, and returns only the response-model-enforced V0.1 `AssessmentRefEnvelope`. Full pillar, finding, suitability, and report-wrapper fields remain available only through authenticated tenant-scoped assessment dereference.
  The compact boundary requires all canonical envelope identity fields, `cache_policy: summary_snapshot`, and nonempty `external_uri`, `snapshot_id`, and `version`; rejects unknown envelope or nested-ref fields; validates the bounded projection through the strict contract binding; and requires `org_id`, `workflow_ref.ref.organization_id`, and the authenticated tenant to match before scoring or persistence.
- Both direct workflow endpoints accept optional `workflow_evidence` to deepen pillar scoring from source evidence. This supports explicit `control_coverage` (`approval_gate`, `decision_logging`, `evidence_retention`, `exception_handling`, `periodic_review` with `missing|documented|operating|verified`) and `evidence_posture` (`control_evidence_coverage_percent`, `freshest_evidence_age_days`, `audit_trail_complete`, `linked_artifacts`), with legacy approval/decision counts retained as fallback.
